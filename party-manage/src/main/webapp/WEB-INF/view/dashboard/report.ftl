...
<@master.masterFrame pageTitle=["报表","报表详细"]>
	<div class="panel panel-default">
		<#if (response.sqlConditionList)?? && response.sqlConditionList?size gt 0>
		<div class="panel-body">按条件查看
			<form id="searchForm" class="form-inline no-margin" action="${basePath}/report/report" method="post">
				<#list response.sqlConditionList as condition>
					<#if (condition.joinRelation)?? && (condition.logicType)?lower_case=="between" && condition.isDate>
						...
						<div class="form-group">
							...
							<div>
								...
								data-parsley-group="groupTemp" >
									...
									<#list groupFieldList as groupField >
										<#if !groupField.isStatistics>
											<#if groupField.fieldId?? && fieldId==groupField.fieldId>
												<option value="${groupField.fieldId}" selected>${groupField.showName}</option>
											<#else>
												<option value="${groupField.fieldId}">${groupField.showName}</option>
											</#if>
										</#if>	
									</#list>
								</select>
