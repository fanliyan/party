...
public class AuthorityAnnotationInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (authController == null) {
            Message.showError(request, response, "此模块未做过权限设置，禁止访问，请联系技术部", false);
        } else if (authMethod != null && !authMethod.mustLogin()) {
            result = true;
        } else {
            // 未登录
            UserModel loginUserModel = AuthHelper.getLoginUserModel(request);
            if (loginUserModel != null) {
                ...
                if (moduleId != -1) {// 说明有权限认证 或者是报表
                    // 检查模块权限
                    if (!AuthHelper.hasModule(loginUserModel, moduleId)) {
                        Message.showError(request, response, "禁止访问！您无权访问此模块！", false);
                        return false;
                    } else {
//						// 当前ModuleId和GroupID写入Cookie
                        ModuleModel model = AuthHelper.getModel(loginUserModel, moduleId);
                        if (model != null) {
